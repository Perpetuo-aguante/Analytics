...
import { deleteSubscriberUpload, listSubscriberUploads, type SubscriberUploadBatch } from "./subscriber-actions";

export function SubscribersManageUploads() {
  const [uploads, setUploads] = useState<SubscriberUploadBatch[] | null>(null);
  ...
  function load() {
    startTransition(async () => {
      const result = await listSubscriberUploads();
  ...
  function handleDelete(snapshotDate: string) {
    startTransition(async () => {
      const result = await deleteSubscriberUpload(snapshotDate);
      if (result.error) {
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-ink-muted">
        Cada carga agrupa todos los suscriptores subidos con la misma fecha de snapshot. Eliminar una carga borra
        sus métricas; los suscriptores que solo existían por esa carga también se eliminan.
      </p>
        ...
        <ul className="panel divide-y divide-line">
          {uploads.map((upload) => (
            <li key={upload.snapshotDate} className="flex items-center justify-between gap-4 px-4 py-3 text-sm">
              <span>
                {upload.snapshotDate}{" "}
                <span className="text-ink-muted">· {upload.subscriberCount.toLocaleString("es")} suscriptores</span>
              </span>
